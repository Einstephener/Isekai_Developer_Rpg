...
internal class Program
{
    ...
    static void DisplayInventory(Character player)
    {
        ...
        for (int i = 0; i < player.Inventory.Count; i++)
        {
            ...
            Console.WriteLine($"{i + 1}. {equippedStatus} \t {item.ItemName} \t | {item.ItemDescription}"); //아이템 부가 정보(추가예정)
            Console.WriteLine();
        }
        Console.WriteLine();
        Console.WriteLine("============================================================================");
...
public class Item : IItem
{
    ...
    public Item(string itemName, int itemAtk, int itemDef, int itemGold, string itemDescription)
    {
        ItemName = itemName;
        ItemAtk = itemAtk;
        ItemDef = itemDef;
